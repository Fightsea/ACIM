...
export default function useContent({ volume, chapter, section, paragraph }) {
  ...
  const hanhleToggleHightlight = idx => {
    const h = volume !== 'W' ? dbHighlight?.[volume]?.[chapter]?.[section]?.[paragraph] : dbHighlight?.[volume]?.[chapter]?.[paragraph];
    const set = new Set(h);
    if (set.has(idx)) {
      set.delete(idx);
    } else {
  ...
  return { volumes, chapters, sections, paragraphs, sentences, ready, lastRead, toggleHightlight: hanhleToggleHightlight };
}
